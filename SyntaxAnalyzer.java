...
public class SyntaxAnalyzer {
    ...
    ************/
    ...
    private void checkRest(LexicalAnalyzer.Token token) throws IOException {
        ...
        switch(token.type){
            ...
            case 8:     
                addToTable(token.lexeme);   // CONSTTOK
                switch((t = getNextToken()).type) {
                    case 3:     
                        addToTable(t.lexeme);   // BASTYPTOK
    ...
    private boolean checkStat(LexicalAnalyzer.Token token) throws IOException {
        ...
        switch(token.type){
            ...
            case 7: //  BEGIN  Rule 19: blockstat --> declpart   BEGINTOK   stats   ENDTOK  ';'
                appendLMD("12");
                appendLMD("21");    // Rule 20: declpart --> empty
    ...
    *****************/
    ...
    private void checkWriteStat(LexicalAnalyzer.Token nextToken) throws IOException {
        ...
        LexicalAnalyzer.Token t;
        if( nextToken.type == 21 ) {    //  '('
            // Rule 22: writeexp --> STRLITTOK
            if( (t = getNextToken()).type == 26) {   //  STRLITTOK
                addToTable(t.lexeme);
                appendLMD("22");
                t = getNextToken();
            } else {  
                //  Rule 23: writeexp --> expression
                appendLMD("23");
    ...
    ************/
    ...
    private boolean checkTerm(LexicalAnalyzer.Token nextToken) throws IOException {
        ...
        if(checkRelFactor(nextToken)) {  // relfactor
            ...
            //  no call to {termprime} here, {termprime} is called from {relFactor}'s 
            //  child {factorprime} in the event that the next Token is "empty", 
            //  meaning it does not belong inside an {expression}.
        } else { 
    ...
    ***************/
    ...
    // only called by nodes which have calls directly to {express}
